...
def _add_values(day_of_month: int, start: str, end: str, duration_plus: int):
    ...
    if c_start.String != "" :
        c_duration.Value = round( float(c_duration.String.replace(',', ''))  +  duration_plus,2) 

        c_duration.Value = round( c.duration.Value  +  duration_plus,2) 
...
def _add_from_gcal(month:int , query=""):
    ...
    day_dict = dict()
    for i in events:
        print( f"{i.start.day}.{i.start.month} {i.start.hour}:{i.start.minute:02} - {i.end.hour}:{i.end.minute:02}\t{i.summary}")
    ...
    for i in day_dict.values():
        ...
        if len(i) > 1:
            for j in i:
                _add_values(day_of_month=j.start.day, start=f"{j.start.hour}:{j.start.minute:02}",end= f"{j.end.hour}:{j.end.minute:02}", duration_plus= (j.end -j.start).total_seconds()/3600 )
        ...
        # if just one appoinment in list(= just one appoinment at this day), add just the times to sheet, and let libreoffice calculate the duration field 
            ...
            for j in i:
                _add_times(day_of_month=j.start.day, start=f"{j.start.hour}:{j.start.minute:02}",end= f"{j.end.hour}:{j.end.minute:02}" ,note=j.summary)
...
def add_from_gcal(query="Nachhilfe"):
    ...
    if (choice=="n" or choice == "no"):
        ...
        active_sheet.getCellRangeByName(CELL_MONTH_YEAR).String = f"{month:02}/{t.year})"
